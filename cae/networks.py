...
class MultiClassDis(nn.Module):
    ...
    def _make_net(self):
        ...
        for i in range(self.n_layer - 1):
            ...
            dim *= 2
        cnn_x += [ResBlocks(2, dim, norm=self.norm, activation=self.activ, pad_type=self.pad_type)]
...
class CAEGen(nn.Module):
    ...
    def __init__(self):
        ...
        # style encoder
        self.enc_style = StyleEncoder(4, 3, 64, 8, norm='none', activ='relu', pad_type='reflect')

        # content encoder
        self.enc_content = ContentEncoder(2, 6, 3, 64, 'in', activ='relu', pad_type='reflect')

        self.dec = Decoder(2, 6, self.enc_content.output_dim, 3, res_norm='adain', activ='relu',
                           pad_type='reflect')

        # MLP to generate AdaIN parameters
        self.mlp = MLP(8, self.get_num_adain_params(self.dec), 256, 3, norm='none', activ='relu')
